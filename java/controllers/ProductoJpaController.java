 ...
 * @return 
 */
    public Productos findProducto(Integer id) {
        EntityManager em = getEntityManager();
        try {
    ...
    try {
        Query query = em.createQuery("SELECT COUNT(p) FROM Productos p WHERE p.idProducto = :id");
...
public static boolean productNameExists(String nombre) {
    EntityManagerFactory emf = Persistence.createEntityManagerFactory("persistence"); 
    EntityManager em = emf.createEntityManager();
    try {
        Query query = em.createQuery("SELECT COUNT(p) FROM Productos p WHERE p.nombre = :nombre");
